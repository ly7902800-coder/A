...
export function listConnectors():Connector[]{return listPlatformTargets().map(p=>({platformId:p.id,auth:p.auth[0],status:p.id==="google"?"ready":"credential_required",capabilities:p.capabilities}));}
