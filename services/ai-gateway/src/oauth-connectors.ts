...
export type OAuthPlatform = "github" | "cloudflare" | "figma";
...
export function listOAuthPlatforms() {
  return (["github", "cloudflare", "figma"] as OAuthPlatform[]).map((platform) => {
...
export function startOAuth(platform: OAuthPlatform, redirectUri: string): OAuthStart {
  ...
  url.searchParams.set("scope", c.scopes.join(platform === "figma" ? "," : " "));
