...
export function startOAuth(platform:OAuthPlatform,redirectUri:string):OAuthStart{
  ...
  if(platform==="google"){url.searchParams.set("access_type","offline");url.searchParams.set("include_granted_scopes","true");url.searchParams.set("prompt","consent");}
...
export async function finishOAuth(platform:OAuthPlatform,code:string,state:string,redirectUri:string){
  ...
  return{platform,accessToken:typeof token.access_token==="string"?token.access_token:undefined,refreshToken:typeof token.refresh_token==="string"?token.refresh_token:undefined,expiresIn:typeof token.expires_in==="number"?token.expires_in:undefined,scope:typeof token.scope==="string"?token.scope:undefined};
}
