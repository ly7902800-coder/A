...
const URLS: Record<ExtendedProvider, string> = {
...
function secret(provider: ExtendedProvider) {
  const names: Record<ExtendedProvider, string> = {
    xai: "XAI_API_KEY",
    openai: "OPENAI_API_KEY",
    anthropic: "ANTHROPIC_API_KEY",
    gemini: "GEMINI_API_KEY"
  };
  return process.env[names[provider]];
}

function modelOf(model: string) {
  return model.replace(/^(xai|openai|anthropic|gemini)\//, "");
}
...
export async function chatExtended(request: ChatRequest): Promise<ChatResponse> {
  ...
  if (provider === "gemini") {
    ...
    const response = await fetch(`${URLS.gemini}/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(key)}`, {
      method: "POST",
  ...
  if (provider === "anthropic") {
    ...
    const response = await fetch(URLS.anthropic, {
      method: "POST",
      headers: {
        "x-api-key": key,
